    ...
    <script>
        ...
        $(function () {
            ...
            <c:forEach var="item" items="${model.listModel}">
                ...
                $('#table').append('<tr class="data"><td>' + i + '</td><td>${item.title}</td><td>${item.shortDescription}</td><td><a class="nav-link" href="${editURL}"><i class="fas fa-edit"></i></a></td><td><input id="checkbox_${item.id}" type="checkbox" name="id" value="${item.id}"></td></tr>');
                i++;
            </c:forEach>
...
<body>
    <div>
        <ul style="list-style: none; float: right; padding-left: 0; margin-right: 70px">
            <li><a style="font-size: 30px; margin-right: 20px;" href="<c:url value="/admin-blog?type=edit"/>" title="Create"><i class="fas fa-folder-plus"></i></a></li>
            <br>
